    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            let goals = [
                ("Lose Weight", "Image1"),
                ("Gain Muscles", "Image2"),
                ("Weight-lifting", "Image3"),
                ("Diet", "Image4")
            ]
            
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 16) {
                ForEach(goals, id: \.0) { goal, image in
                    GoalButton(goal: goal, image: image, selectedGoal: $selectedGoal)
                }
            }
            
            Button(action: {
                selectedGoal = "Other"
                otherBtnTapped.toggle()
            }) {
                HStack {
                    Image(systemName: selectedGoal == "Other" && otherBtnTapped ? "checkmark.circle.fill" : "circle") // Logic adjusted for clarity
                        .foregroundStyle(selectedGoal == "Other" && otherBtnTapped ? Color.colorPurple : Color.primary)
                    Text("Other")
            ...
            if selectedGoal == "Other" {
                TextField("Type here", text: $otherGoal)
                    .textFieldStyle(RoundedBorderTextFieldStyle()) // Using a standard style for example
                // .formFieldStyle() // Assuming formFieldStyle is a custom modifier
            }
            ...
            .simultaneousGesture(TapGesture().onEnded {
                ...
                viewModel.fitnessGoal = selectedGoal == "Other" ? otherGoal : selectedGoal
...
struct TraineeData2View: View {
    @State private var fitnessLevel: String = "" // Local state for selection
    // Access the shared ViewModel from the environment
    @EnvironmentObject var viewModel: UpdateClientViewModel
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
                ...
                Button(action: { fitnessLevel = level }) {
                    HStack {
                        Image(systemName: fitnessLevel == level ? "checkmark.circle.fill" : "circle")
                            .foregroundStyle(fitnessLevel == level ? Color.colorPurple : Color.primary)
                        Text(level)
                            .foregroundColor(.primary)
                    }
                }
            ...
            .simultaneousGesture(TapGesture().onEnded {
                ...
                print("✅ Fitness level set to: \(viewModel.fitnessLevel)")
            })
        }
        .padding()
        .toolbar {
...
struct TraineeDataAgeView: View {
    @State private var age: Int = 14 // Local state for picker
    // Access the shared ViewModel from the environment
    @EnvironmentObject var viewModel: UpdateClientViewModel
        ...
        VStack(spacing: 20) {
            ...
            Picker("Age", selection: $age) {
                ForEach(10...60, id: \.self) { value in
                    Text("\(value)")
                        .font(.system(size: 30))
                    // .foregroundColor(.white) // Consider contrast if background changes
                        .tag(value)
                }
            }
...
struct TraineeDataHeightWeightView: View {
    @State private var selectedHeight: Int = 155
    @State private var selectedHeightUnit: String = "cm"
    @State private var selectedWeight: Int = 74
...
struct TraineeData5View: View {
    ...
    var body: some View {
        ...
        VStack(spacing: 20) {
            ...
            // Navigate to AuthView on success (assuming AuthView exists)
...
struct GoalButton: View {
    let goal: String
    @State var image: String // Assuming image name corresponds to an asset
    @Binding var selectedGoal: String
    
    var body: some View {
        Button(action: { selectedGoal = goal }) {
            VStack(alignment: .center, spacing: 8) { // Centered alignment
                HStack {
                    Image(systemName: selectedGoal == goal ? "checkmark.circle.fill" : "circle")
                        .foregroundStyle(selectedGoal == goal ? Color.colorPurple : Color.primary)
                    Spacer() // Push checkmark to the leading edge
                }
                .padding(.horizontal) // Add padding for the checkmark
                
                Image(image) // Use the image name directly
                    .resizable()
                    .scaledToFit()
                    .frame(height: 80) // Example frame
            ...
            .padding()
            .frame(maxWidth: .infinity, minHeight: 160) // Use maxWidth and minHeight
            .background(RoundedRectangle(cornerRadius: 12).stroke(selectedGoal == goal ? Color.colorPurple : Color.gray.opacity(0.3), lineWidth: selectedGoal == goal ? 2 : 1))
        }
    }
}
